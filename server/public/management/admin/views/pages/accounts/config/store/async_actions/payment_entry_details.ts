...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    const response = await axios.get(`${end_point}/${api_prefix}/entry-details/${param.id}`);
...
export const payment_entry_details = createAsyncThunk<ReturnType, PayloadType, ThunkArgument>(
    `${store_prefix}/entry-details`,
    fetch_api,
);
