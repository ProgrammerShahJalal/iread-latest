...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    const response = await axios.get(`${end_point}/${api_prefix}/${param.id}/approve-payment`);
...
export const approve_payment = createAsyncThunk<ReturnType, PayloadType, ThunkArgument>(
    `${store_prefix}/approve_payment`,
    fetch_api,
);
